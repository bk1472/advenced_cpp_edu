...
class Test
{
	Data data;
public:
	// call by value - 복사본 생성으로 좋지 않다.
	//void setData(Data d) { data = d; }

	// const & 사용 - C++98에서 가장 중요했던 기술
	...
	// 방법 1 보다 느리긴 하지만 "한개의 함수"로 move를 지원할 수 있게 된다.
	//void setData(Data d) { data = std::move(d); }

	// 방법 3. "방법1"의 2개의 함수를 자동 생성 할수 없을까 ? - "forwarding reference"
