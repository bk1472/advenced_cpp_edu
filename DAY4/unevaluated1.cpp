...
int main()
{
	...
	int n1 = sizeof(a);         // 4
	int n2 = sizeof(int);       // 4
	int n3 = sizeof(&foo);      // 8 - 함수 주소의 크기(즉, 포인터 타입의 크기)

//	int n4 = sizeof(foo);       // C에서는 foo 을 함수의 주소로 암시적 치환 하지만
	                            // C++ 에서는 error.
								// error. - 함수 자체의 크기는 구할 수 없다.
								// 이유 - 컴파일러는 함수 선언만으로도 함수를 호출 할수 있는데
								// 힘수의 크기를 구하려면 "완벽한 구현이 제공" 되어야 한다.

	// 핵심 : sizeof(함수호출식) : 함수호출의 결과로 나오는 타입의 크기.
	int n5 = sizeof(foo(1,2));  // foo의 반환 타입인 int의 크기 - 4

	// 평가되지 않은 표현식(unevaluated expression)
	// => 함수 호출식 같은 표현식이 실제로는 실행되지 않고, 컴파일 시간에만 활용되는 것(함수의 선언부만 가지고 있어도 됨)
