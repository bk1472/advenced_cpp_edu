...
//	pair(T&& a, U&& b) : first(std::move(a)), second(std::move(b)) {}
	...
	// setData의경우 인자가 한개이므로 2개의 setter를 제공하면 되지만
	// par의 경우 4개의 생성자가 필요!!
...
//	pair(A&& a, B&& b) : first(std::forward<A>(a)), second(std::forward<B>(b)) {}
	...
	// 생성자는 "반환 타입이 없습니다."
//	template<typename A, typename B>
//	std::enable_if_t< std::is_convertible_v<A, T> && std::is_convertible_v<B, U> >
//	pair(A&& a, B&& b) : first(std::forward<A>(a)), second(std::forward<B>(b)) {}
...
int main()
{
   ...
   // 아래 코드는 아예 생성자 자체가 c++코드로 생성되지 않게 하는 것이 좋다
//	pair<std::string, std::string> p2(1, 3.4); // 이 경우의 현상을 생각 해 봅시다.
									// 1. pair(int, double) 버전의 생성자가 c++ 코드로 생성
									// 2. (1)에서 만든 생성자 코드를 컴파일 할 때 에러..
									//    "first(1)" 코드가 컴파일 실패
