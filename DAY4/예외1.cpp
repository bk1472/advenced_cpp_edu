...
// 예외 : try~catch, thro 키워드.

// noecept
// 1. 특정 함수가 예외가 있는지 없는지 조사할때 사용 : noexcept( 함수 호출식 )
// 2. 함수 선언의 뒤쪽에 붙여서 "함수가 예외 없음"을 알릴 때 사용
void f1() noexcept
{
	int a = 10;
	a = 20;
...
void f2()
{
	// 이 안에서 발생하는 모든 예외를 catch 로 잡아서 처리 후
	// 외부에 다시 throw 하지 않는 경우는 noexcept를 가능.
	int* p = new int[10]; // 메모리 부족시 std::bad_alloc 이라는 예외가 throw 됩니다.
	// if(p == 0) {} /이코드는 C에서나 사용됨
	delete[] p;
}
