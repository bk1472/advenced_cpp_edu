...
{
	auto ret1 = foo(10, 20);
	             // int& 이지만 auto는 참조를 제거하고 타입결정
				 // int ret1 = foo(10, 20);

	// decltype(함수호출식) : 함수 호풀의 결과로 나오는 타입 조사(반환타입)
	//                        실제 함수가 호출되는 것은 아님.
	...
	decltype(auto) ret3 = foo(10, 20); // 실전에서 쓰는 기법
	          // int& ret3 = foo(10, 20); 의 의미

	// 아래와 같이 forwarding reference 도 사용할 수 있다
